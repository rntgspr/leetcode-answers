...
var closeStrings = function (word1, word2) {
  ...
  for (let char of mapWord1.keys()) {
    if (!mapWord2.has(char)) return false;
  }

  const mapWord1Values = [...mapWord1.values()].sort((a, b) => a - b);
  const mapWord2Values = [...mapWord2.values()].sort((a, b) => a - b);
  return mapWord1Values.every((item, index) => item === mapWord2Values[index]);
};
